package com.lazite.assist.excel;
...
public class ExcelUtil {
	...
	@SuppressWarnings("unchecked")
	public <T> List<T> readExcel(String xlsPath,Class cls) throws IOException{
		objClass=cls;
		File file=checkFileIsExist(xlsPath);		
		//获取扩展名
		String ext = xlsPath.substring(xlsPath.lastIndexOf(".")+1);
					...
					out("第"+(i+1)+"行：",false);
					 ...
					 for (int j = 0; j < row.getLastCellNum(); j++) {
						 ...
						 if(decilfields!=null&&decilfields.length!=0){
							...
							out("set对象属性为空,已按照普通对象属性予以赋值",false);
...
//	}
	 ...
	 * @throws: TODO
	 */
	private File checkFileIsExist(String filePath){
		if (filePath.equals("")){
			out("文件路径不能为空！",true);
		}else{
	 ...
	 * @throws IOException 
	 */
	public Workbook loadTemplate(String templateURL){
		File file=checkFileIsExist(templateURL);		
		String ext = templateURL.substring(templateURL.lastIndexOf(".")+1);
		//this.extType=ext;
		Workbook wb=null;
		try{
			...
			if("xls".equals(ext)){
				wb = new HSSFWorkbook(fs);//生成 2003版，xls格式
			}else if("xlsx".equals(ext)){
				wb=new XSSFWorkbook(fis);//生成Excel 2007版，xlsx格式
			}

		}catch (Exception e){
			out("loadTemplate()方法异常，文件加载失败：",true);
		}
		return wb;
	}
	
	public Workbook loadTemplate(InputStream is){
		Workbook wb=null;
		try{
			POIFSFileSystem pfs = new POIFSFileSystem(is);
			if("xls".equals(this.extType)){
				wb = new HSSFWorkbook(pfs);//生成 2003版，xls格式
			}else if("xlsx".equals(this.extType)){
				wb = new XSSFWorkbook(is);//生成Excel 2007版，xlsx格式					
			}
		}catch (Exception e){
			out("loadTemplate()方法异常，文件加载失败：",true);
		}
	...
	public List<Sheet> cloneSheets(Workbook wb){
		if(wb!=null){
			List<Sheet> sheets=new ArrayList<Sheet>();
			for (int i = 0; i < wb.getNumberOfSheets(); i++) {
				sheets.add(wb.cloneSheet(i));
			}
			return sheets;
		}
		return null;
...
//	public Sheet writeInSheet(Sheet sheet,int beginRow,int beginCell,String value){
